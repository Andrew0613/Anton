...
func CommandSurface() []CommandProbe {
	return []CommandProbe{
		...
		{Command: "task-state", Usage: "anton task-state <init|pulse|check|env|service|freshness|sync-card|close|reopen|retarget|import> [--json]", JSONSupport: true},
		{Command: "task", Usage: "anton task <resolve|list> [--json]", JSONSupport: true},
		{Command: "run", Usage: "anton run <init|status|task|audit|close> [--json]", JSONSupport: true},
